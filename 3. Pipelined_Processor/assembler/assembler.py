...
# Below code defines utility functions (used throughout the assembler)
...
def offset(literal):  # Automatically checks/outputs immediates and whether they're HEX or DEC
    if "x" in literal:
...
def pack_imm(opcode, imm, rs, rt):  # Bit-packing function for I-type instructions
...
def pack_reg(opcode, rd, rs, rt):  # Bit-packing function for R-type instructions
...
def orig(statement):  # Directive to change the current memory location of the CPU
    global orig_address
...
def word(statement):  # Directive to define some memory variable
    ...
    if hex_word in labels:
        hex_word = int(labels[hex_word])
    elif hex_word in names:
        hex_word = int(names[hex_word])
    else:
        hex_word = offset(hex_word)
...
def branch(statement):
    ...
    imm = params[2].strip()
    if imm in labels:
        imm = int(labels[imm]) - int(statement[0]) - 1
    elif imm in names:
        imm = int(names[imm]) - int(statement[0]) - 1
    else:
        imm = offset(imm)
    output_statements.append([statement[0],
...
def main():
    ...
    with open(str(input_path), "rtU") as input_file:
        ...
        for line in input_file:
            ...
            for char in line.strip():
                if char == ';':
                    break  # Identified an assembler comment, skip line
    ...
    if "." in str(input_file.name):
        ...
        output_path = str(input_file.name) + ".mif"
    ...
    if output_statements[0][0] > 0:
        end_address = str("{0:02X}".format(output_statements[0][0] - 1)).zfill(4)
        output_file.write("[" + "0000" + ".." + end_address + "] : DEAD;\n")
    ...
    for i in range(len(output_statements) - 1):
        ...
        if pc[0] != ir[0] + 1:
            start_address = str("{0:02X}".format(ir[0] + 1)).zfill(8)
            end_address = str("{0:02X}".format(pc[0] - 1)).zfill(8)
            output_file.write("[" + start_address + ".." + end_address + "] : DEAD;\n")
    ...
    if output_statements[-1][0] < 0x3FFF:
        start_address = str("{0:02X}".format(output_statements[-1][0] + 1)).zfill(4)
        output_file.write("[" + start_address + ".." + "3FFF" + "] : DEAD;\n")
